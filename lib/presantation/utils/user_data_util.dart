...
class UserDataUtil {
  ...
  static String? _parseData(String data, DataType type) {
    ...
    if (type == DataType.RAW) {
      ...
      if (address.isValid()) {
        return data;
      } else if (NearWallet.parseUrl(data) != null) {
        return data;
      }
    } else if (type == DataType.SEED) {
      // Check if valid seed
      if (NearSeeds.isValidSeed(data)) {
        return data;
      }
    }
    return null;
  ...
  static Future<void> setSecureClipboardItem(String value) async {
    if (Platform.isIOS) {
      ...
      setStream.cancel();
          Future<dynamic> delayed = Future.delayed(const Duration(minutes: 2));
      delayed.then((_) {
        return true;
      });
      setStream = delayed.asStream().listen((_) {
        Clipboard.getData("text/plain").then((data) {
          if (data != null &&
              data.text != null &&
              NearSeeds.isValidSeed(data.text)) {

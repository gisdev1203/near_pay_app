...
class PinScreen extends StatefulWidget {
  ...
  const PinScreen(this.type,
      {super.key, this.description = "", this.expectedPin = "", required this.pinScreenBackgroundColor});
...
class _PinScreenState extends State<PinScreen>
  ...
  void initState() {
    ...
    _dotStates = List.filled(_pinLength, AppIcons.dotty);
  ...
  bool _setCharacter(String character) {
    ...
    for (int i = 0; i < _dotStates.length; i++) {
      if (_dotStates[i] == AppIcons.dotty) {
        setState(() {
  ...
  void _backSpace() {
    if (_dotStates[0] != AppIcons.dotty) {
      int lastFilledIndex;
      for (int i = 0; i < _dotStates.length; i++) {
        if (_dotStates[i] == AppIcons.dotfilled) {
          if (i == _dotStates.length ||
              _dotStates[i + 1] == AppIcons.dotty) {
            lastFilledIndex = i;
            break;
          }
        }
      ...
      setState(() {
        _dotStates[lastFilledIndex] = AppIcons.dotty;
        if (_awaitingConfirmation) {

...
class SlideToDismissDrawerDelegate extends SlideToDismissDelegate {
  const SlideToDismissDrawerDelegate({
    ...
    required super.onResize,
    required super.onDismissed,
    super.resizeDuration,
    super.crossAxisEndOffset,
    required super.onWillDismiss,
    super.closeOnCanceled,
  });
...
class SlidableDelegateContext {
  ...
  Positioned createPositioned({required Widget child, required double extent, required double position}) {
    return Positioned(
...
class SlidableController {
  SlidableController({
    required this.onSlideAnimationChanged,
    required this.onSlideIsOpenChanged,
  });
...
class Slidable extends StatefulWidget {
  ...
  Slidable({
    ...
    bool enabled = true,
    required SlideToDismissDelegate slideToDismissDelegate,
    required SlidableController controller,
    required Function onTriggered,
    required Function onAnimationChanged, required actionPane,
  }) : this.builder(
          key: key,
          child: child,
          delegate: delegate,
  ...
  const Slidable.builder({
    ...
    required this.secondaryActionDelegate,
    ...
    this.enabled = true,
    required this.slideToDismissDelegate,
...
class SlidableState extends State<Slidable>
  ...
  late AnimationController _actionsMoveController;
  @override
  ...
  void dismiss({required SlideActionType actionType}) {
    if (dismissible) {
      _dismissing = true;
  ...
  late StreamSubscription<dynamic> _callbackStream;
  ...
  void _handleShowAllActionsStatusChanged(AnimationStatus status) {
    ...
    if (_dragExtent < 0 && status == AnimationStatus.completed && actionsMoveAnimation.value >= 1.0 && !_callbackComplete) {
      ...
      var delayed = Future.delayed(const Duration(milliseconds: 150));
      delayed.then((_) {

...
import 'model/response/subscribe_response.dart';
...
class AccountService {
  ...
  Future<void> reconnectToService() async {
    ...
    Future<dynamic> delayed = Future.delayed(const Duration(seconds: 3));
    delayed.then((_) {
      return true;
    });
  ...
  void removeSubscribeHistoryPendingFromQueue() {
    if (_requestQueue.isNotEmpty) {
      ...
      for (var requestItem in toRemove) {
  ...
  void clearQueue() {
    ...
    _requestQueue.clear();
    // Re-queue requests
    for (var requestItem in reQueue) {
      requestItem.isProcessing = false;
  ...
  Future<AccountInfoResponse> getAccountInfo(String account) async {
    AccountInfoRequest request = AccountInfoRequest(account: account, action: '');
    dynamic response = await makeHttpRequest(request as BaseRequest);
    if (response is ErrorResponse) {
      if (response.error == "Account not found") {
        return AccountInfoResponse(unopened: true, frontier: '', openBlock: '', representativeBlock: '', balance: '', blockCount: null);
      }
  ...
  Future<PendingResponse> getPending(String account, int count,
      {required String threshold, bool includeActive = false}) async {
    threshold = threshold;
  ...
  Future<AccountHistoryResponse> requestAccountHistory(String account,
    ...
    AccountHistoryRequest request =
        AccountHistoryRequest(account: account, count: count, action: '');
    dynamic response = await makeHttpRequest(request as BaseRequest);
    if (response is ErrorResponse) {
      throw Exception("Received error ${response.error}");
    }
  ...
  Future<ProcessResponse> requestReceive(String representative, String previous,
    ...
    StateBlock receiveBlock = StateBlock(
        ...
        link: link,
        account: account,
        privKey: privKey, localCurrencyValue: '');
  ...
  Future<ProcessResponse> requestOpen(
      ...
      {required String representative}) async {
    representative =
        representative;

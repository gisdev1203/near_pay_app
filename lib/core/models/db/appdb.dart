...
class DBHelper {
  ...
  Future<List> getContactsWithNameLike(String pattern) async {
    var dbClient = await db;
  ...
  // Accounts
  Future<List> getAccounts(String seed) async {
    var dbClient = await db;
    List<Map> list =
    ...
    for (int i = 0; i < list.length; i++) {
      accounts.add(Account(
          ...
          balance: list[i]["balance"], address: ''));
    }
    for (var a in accounts) {
      a.address = NearUtil.seedToAddress(seed, a.index);
    }
    return accounts;
  }

  Future<List> getRecentlyUsedAccounts(String seed,
      {int limit = 2}) async {
    var dbClient = await db;
    List<Map> list = await dbClient.rawQuery(
  ...
  Future<Account> addAccount(String seed, {required String nameBuilder}) async {
    var dbClient = await db;
    Account account;
    await dbClient.transaction((Transaction txn) async {
      ...
      account = Account(
          ...
          lastAccess: 0,
          selected: false,
          address: NearUtil.seedToAddress(seed, nextIndex), id: 0, balance: '');
          //id :0 idk if its correct
      await txn.rawInsert(
  ...
  Future<Account?> getSelectedAccount(String seed) async {
    ...
    String address =
        NearUtil.seedToAddress(seed, list[0]["acct_index"]);
